...
import { ArrowLeft, ExternalLink, Github, BarChart3, TrendingUp, Shield, Search, Newspaper, Star, Users } from "lucide-react";
import { Link } from "react-router-dom";
import FighterBackground from "@/components/FighterBackground";
import Navbar from "@/components/Navbar";

const features = [
  { icon: BarChart3, title: "Dashboard Overview", desc: "Comprehensive portfolio overview with total value, profit/loss, overall return, and AI-powered diversification tips." },
  { icon: TrendingUp, title: "Portfolio Management", desc: "Track stock holdings, monitor performance, manage positions, and view detailed analytics including Sharpe Ratio and VaR." },
  { icon: Shield, title: "Stock Trading", desc: "Seamless interface for buying and selling stocks with real-time data and instant execution." },
  { icon: Search, title: "Stock Screener", desc: "Powerful filtering tool for discovering stocks based on criteria like P/E ratio, market cap, volume, and sector." },
  { icon: Newspaper, title: "Financial News", desc: "Dedicated news section aggregating the latest financial news to keep traders informed and ahead of the market." },
  { icon: Star, title: "Watchlist", desc: "Create and manage a personalized watchlist of stocks with real-time price updates and alerts." },
  { icon: Users, title: "JWT Authentication", desc: "Secure user authentication with JWT including login, signup, and password reset functionality." },
];
      ...
      <section className="pt-32 pb-16 relative">
        <div className="container mx-auto px-6 max-w-5xl">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
            <Link to="/" className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors font-heading text-sm uppercase tracking-wider mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            ...
            <p className="text-muted-foreground text-lg leading-relaxed max-w-2xl mb-8">
              A modern, data-driven stock trading application built with Next.js 15. Provides a comprehensive suite of tools for tracking and analyzing the stock market, managing portfolios, and making informed trading decisions.
            </p>
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => (
              ...
              >
                <f.icon className="text-primary mb-4 group-hover:scale-110 transition-transform" size={24} />
                <h3 className="font-display font-bold uppercase mb-2">{f.title}</h3>
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="glass-card rounded-lg overflow-hidden border border-primary/20">
            {structure.map((s, i) => (
              ...
              >
                <code className="font-mono text-primary text-sm font-bold w-32 shrink-0">{s.dir}</code>
        ...
        <div className="container mx-auto px-6 max-w-5xl">
          ...
          <div className="flex flex-wrap gap-3">
            {tech.map((t, i) => (
              ...
              >
                {t.label}
                <span className="ml-2 text-xs text-primary opacity-70">{t.cat}</span>
              </motion.span>
            ))}
          </div>
        </div>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6 max-w-5xl text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="font-display text-4xl font-bold uppercase mb-6">Trade <span className="energy-gradient-text">Smarter</span></h2>
